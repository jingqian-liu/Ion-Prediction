...
from pytorch3dunet.unet3d.se import GridAttentionBlock3D
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, x_channels, g_channels, conv_kernel_size=3, scale_factor=2, basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, padding=1, upsample='default',
                 dropout_prob=0.1, is3d=True, Attention=False):
        ...
        if upsample is not None and upsample != 'none':
            ...
            # concat joining
            self.joining = partial(self._joining, concat=True, attention = True)
        ...
        print(self.Attn)
        if self.Attn == True:
            self.attention = GridAttentionBlock3D(x_channels, g_channels, inter_channels=None, mode='concatenation',sub_sample_factor=(2,2,2))
            self.basic_module = basic_module(out_channels*2, out_channels,
                                         encoder=False,
                                         kernel_size=conv_kernel_size,
                                         order=conv_layer_order,
                                         num_groups=num_groups,
    ...
    def forward(self, encoder_features, x):
        ...
        if self.Attn == True:
            x, attn = self.attention(encoder_features, x)
        else:
    ...
    @staticmethod
    def _joining(encoder_features, x, concat, attention = True):
        if concat:
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order,
                    num_groups, upsample, dropout_prob, is3d, Attention):
    ...
    for i in range(len(reversed_f_maps) - 1):
        ...
        x_feature_num = reversed_f_maps[i + 1]
        g_feature_num = reversed_f_maps[i]

        print("xg feature number", in_feature_num, out_feature_num, x_feature_num, g_feature_num)
        decoder = Decoder(in_feature_num, out_feature_num, x_feature_num, g_feature_num,
                          basic_module=basic_module,
                          conv_layer_order=layer_order,
                          conv_kernel_size=conv_kernel_size,
                          num_groups=num_groups,
